...
function call() {
    ...
    log("[call] RTCPeerConnection object: " + RTCPeerConnection);
    // This is an optional configuration string, associated with
    // NAT traversal setup
    var servers = null;
    // Create the local PeerConnection object
    localPeerConnection = new RTCPeerConnection(servers);
    log("[call] Created local peer connection object localPeerConnection");
    // Add a handler associated with ICE protocol events
    localPeerConnection.onicecandidate = gotLocalIceCandidate;
    // Create the remote PeerConnection object
    remotePeerConnection = new RTCPeerConnection(servers);
    log("[call] Created remote peer connection object remotePeerConnection");
    // Add a handler associated with ICE protocol events...
    ...
    log("[call] Added localStream to localPeerConnection");
    // We're all set! Create an Offer to be 'sent' to the callee as soon
    // as the local SDP is ready. it asks the browser to properly examine
    // the internal state of the PeerConnection and generate an appropriate
    // RTCSessionDescription object, thus initiating the Offer/Answer-state
    // machine.
...
function gotLocalDescription(description) {
    ...
    log("[gotLocalDescription] Offer from localPeerConnection: \n" + description.sdp);
...
function gotRemoteDescription(description) {
    ...
    log("[gotRemoteDescription] Answer from remotePeerConnection: \n" + description.sdp);
