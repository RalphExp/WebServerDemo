...
void WebRTCMain::startPipeline(bool master)
{
  stopPipeline();
...
void WebRTCMain::onMessage(WebsocketClient *client, std::string& message)
{
  ...
  g_print("on message: %s\n", text);
  
  if (g_strcmp0(text, "created") == 0) {
    master_ = true;
  } else if (g_strcmp0(text, "joined") == 0) {
    startPipeline(master_);
  } else if (g_strcmp0(text, "playerDisconnected") == 0) {
    stopPipeline();
  } else {
    parseSdpAndIce(message);
  }
}
